#!/bin/zsh

d=${0:a:h:h}/data/scenario
APP_ID=AKfycbz3geFe7AX7a_grWyvMia4sclhja4AQYhQFg7NZcECHPWo6VKc

t=`zsh -c "ls $d/s*"`
n=`echo "$t"|wc -l`
for (( i=1;i<=$n;i++ ))
do
    tt=`echo "$t"|awk "NR==$i"`
    body=`cat $tt`
    nn=`echo "$body"|wc -l`
    su=`printf %02d $i`
    more=`expr $i`
    moree=`expr $more + 1`
    more=`printf %02d $more`
    file_en=$d/e${su}.ks
    file_zh=$d/c${su}.ks
    echo $file_en
    if [ ! -f $file_en ] && [ ! -f $file_zh ];then
    	for (( ii=1;ii<=$nn;ii++ ))
    	do

    	    body_l=`echo "$body"|awk "NR==$ii"`
    	    if echo "$body_l"|grep -e "^\[" -e "^\*" -e "^\@";then
    	    	echo $body_l >> $file_en
    	    	echo $body_l >> $file_zh
            else
               if echo $body_l|grep -e "^\#";then
    	    	  body_s=`echo $body_l|cut -d '#' -f 2-`
    	    	  en=`curl -sL -d "{\"txt\":\"${body_s}\",\"src\":\"ja\",\"tar\":\"en\"}" https://script.google.com/macros/s/${APP_ID}/exec`
    	    	  zh=`curl -sL -d "{\"txt\":\"${body_s}\",\"src\":\"ja\",\"tar\":\"zh-CN\"}" https://script.google.com/macros/s/${APP_ID}/exec`
            	  echo "; #${body_s}" >> $file_en
            	  echo "; #${body_s}" >> $file_zh
    	    	  echo "#${en}" >> $file_en
    	    	  echo "#${zh}" >> $file_zh
    	    	  echo $en
    	    	  echo $zh
               elif [ -n "$body_l" ];then
    	          body_s=`echo $body_l|cut -d '[' -f -1`
    	          body_e=`echo $body_l|cut -d '[' -f 2-`
    	          en=`curl -sL -d "{\"txt\":\"${body_s}\",\"src\":\"ja\",\"tar\":\"en\"}" https://script.google.com/macros/s/${APP_ID}/exec`
    	          zh=`curl -sL -d "{\"txt\":\"${body_s}\",\"src\":\"ja\",\"tar\":\"zh-CN\"}" https://script.google.com/macros/s/${APP_ID}/exec`
                  echo "; ${body_s}" >> $file_en
                  echo "; ${body_s}" >> $file_zh
    	          echo "${en}[${body_e}" >> $file_en
    	          echo "${zh}[${body_e}" >> $file_zh
    	          echo $en
    	          echo $zh
               fi
    	    fi
    	done
    fi
    sed -i "" "s/\[glink storage=\"s${more}.ks\" text=\"続きを読む\"/\[glink storage=\"e${moree}.ks\" text=\"MORE\"/g" $file_en
    sed -i "" "s/\[glink storage=\"s${more}.ks\" text=\"続きを読む\"/\[glink storage=\"c${moree}.ks\" text=\"MORE\"/g" $file_zh
done
